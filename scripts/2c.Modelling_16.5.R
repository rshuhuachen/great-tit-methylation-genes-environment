...
lmm_out_16.5 <- mclapply(data, function_lmm, mc.cores=40)

#then save as dataframe rather than list
data_lmm_out_16.5 <- do.call(rbind.data.frame, lmm_out_16.5)
...
names(data_lmm_out_16.5)[1] <- "CHR"
data_lmm_out_16.5 <- cbind(data_lmm_out_16.5, chr_pos)
data_lmm_out_16.5 <- data_lmm_out_16.5 %>% relocate("CHR_POS", .after = "CHR") 

#multiple testing correction for p-values using FDR
data_lmm_out_16.5$GeneticNK.qval <- p.adjust(data_lmm_out_16.5$GeneticNK.pval, method="fdr", n = nrow(data_lmm_out_16.5))
data_lmm_out_16.5$FosterNK.qval <- p.adjust(data_lmm_out_16.5$FosterNK.pval, method="fdr", n = nrow(data_lmm_out_16.5))

### Add chromosome names to both genetic and foster dataset

#change chr names
data_lmm_out_16.5.adjchr <- as.data.frame(data_lmm_out_16.5)
data_lmm_out_16.5$chr <- mapvalues(x=data_lmm_out_16.5$chr, 
                              from=levels(chr_names$Code),   
                              to  =levels(chr_names$Name)) 
data_lmm_out_16.5.adjchr$chrName_scaffolds<-as.character(data_lmm_out_16.5.adjchr$chr)
Scaffolds<-grep(pattern = "Scaffold",x = data_lmm_out_16.5.adjchr$chr)
data_lmm_out_16.5.adjchr$chrName_scaffolds[Scaffolds]<-"scaffolds"
data_lmm_out_16.5.adjchr$chrName_scaffolds<-as.factor(data_lmm_out_16.5.adjchr$chrName_scaffolds)
data_lmm_out_16.5.adjchr$chr<-data_lmm_out_16.5.adjchr$chrName_scaffolds
data_lmm_out_16.5.adjchr$chr <- as.character(data_lmm_out_16.5.adjchr$chr)
count(data_lmm_out_16.5.adjchr$chr) # 5851 scaffolds
data_lmm_out_16.5.adjchr$chr <- str_remove_all(data_lmm_out_16.5.adjchr$chr, pattern = "chr")
...
data_lmm_out_sign_genetic <- subset(data_lmm_out_16.5, GeneticNK.qval<0.05 & ICC_GeneticNK > 0.15)
data_lmm_out_sign_genetic_woverdisp <- subset(data_lmm_out_16.5, GeneticNK.qval<0.05 & ICC_GeneticNK > 0.15 & overdisp.lmer_fun.model..2. < quantile(overdisp.lmer_fun.model..2., 0.95))

data_glmm_out_sign_foster <- subset(data_lmm_out_16.5, FosterNK.qval<0.05 & ICC_FosterNK > 0.15)
data_glmm_out_sign_foster_woverdisp <- subset(data_lmm_out_16.5, FosterNK.qval<0.05 & ICC_FosterNK > 0.15 & overdisp.lmer_fun.model..2. < quantile(overdisp.lmer_fun.model..2., 0.95))

data_glmm_out_sign_overlap <- subset(data_lmm_out_16.5, GeneticNK.qval<0.05 & ICC_GeneticNK > 0.15 & FosterNK.qval<0.05 & ICC_FosterNK > 0.15)
data_glmm_out_sign_overlap_woverdisp <- subset(data_lmm_out_16.5, GeneticNK.qval<0.05 & ICC_GeneticNK > 0.15 & FosterNK.qval<0.05 & ICC_FosterNK > 0.15 & overdisp.lmer_fun.model..2. < quantile(overdisp.lmer_fun.model..2., 0.95))
...
save(data_lmm_out_16.5, "/data/ModelOutput.RData") #this file can be found in github directory
...
plot(data_lmm_out_16.5$Var.Perc.Gen, -log(data_lmm_out_16.5$GeneticNK.qval))
cor.test(data_lmm_out_16.5$Var.Perc.Gen, data_lmm_out_16.5$GeneticNK.qval) 
